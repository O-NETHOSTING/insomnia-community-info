...
if(isset($_POST['serverid']))
{
    ...
    {
        echo 'Server vam je suspendovan. Nemo�ete da koristite ovu opciju dok ne uplatite server.';
        die();
    }
}
...
switch ($task)
{
    ...
    case 'server-start':
        ...
        if($server['igra'] == "2")
        {
            ...
            $samprcon = str_replace(" ", "", sampcfg("rcon", $serverid));
            $samprcon = trim(preg_replace('/\s+/', '', $samprcon));

            if($cfgslotovi > $server['slotovi'])
            {
            ...
            else if($samprcon == "changeme")
            {
                echo "Morate promeniti rcon servera (changeme) u server.cfg";
                die();
            }
        }
...
header("Location: gp-server.php?id=".$serverid);
    ...
    case 'server-boost':
        ...
        if($server['boost'] > time())
        {
            ...
            $_SESSION['msg'] = "Slede�i boost mo�ete obaviti tek ".$vreme."";
    ...
    case'server-ftppw':
        ...
        {
            if(!ssh2_auth_password($con, $server['username'], $server['password'])) { echo "Neta�ni podatci za prijavu"; die(); }
            else 
            {           
                $cmd1 = 'passwd';
...
"'.$steamid.'" "'.$pw.'" "'.$privilegije.'" "ca" //'.$komentar.'';      
                ...
                else 
                {
                    ...
                    {               
                        ...
                        {
                            echo "Dogodila se gre�ka prilikom dodavanja admina.";
                            die();
                        }
                        unlink($folder);                                
                    }
        ...
        case'promena-moda':
            ...
            if($stop == "instaliran")
            {
                ...
                $_SESSION['msg'] = "Uspe�no ste promenili va� mod na serveru.";
                header("Location: gp-modovi.php?id=".$serverid);
                die();
            }
            else
        ...
        case'kick-igraca':
            ...
            if($server['igra'] == "1")
            {
                ...
                $_SESSION['msg'] = "Uspe�no ste kikovali igra�a po nicku <z>{$nick}</z>";
                header("Location: gp-server.php?id=".$serverid);
                die();
            }
        break;
            ...
            {           
                ...
                else 
                {
                    ...
                    {               
                        ...
                        {
                            $greska = "Dogodila se gre�ka prilikom dodavanja plugina.";
                        }
                        unlink($folder);                                
                    }
                }
